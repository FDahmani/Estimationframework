...
t_0 = time_conversion.DateTime(1980,1,1,12,0,0).epoch()
...
#Set up environment
bodies_to_create = ["Sun",'Saturn', "Earth",'Jupiter', 'Io', 'Europa','Ganymede','Callisto','Himalia']
...
body_settings.get("Himalia").gravity_field_settings = environment_setup.gravity_field.central(0.28*10**(9)) 

#For propagated moons, voor initial state, Nan lijst veranderen
body_settings.get('Himalia').ephemeris_settings = environment_setup.ephemeris.tabulated_from_existing(environment_setup.ephemeris.direct_spice("Jupiter",global_frame_orientation,"Himalia"),t_0-12*7200,t_end+12*7200,20*60,interpolators.lagrange_interpolation(4))         #Check number of points used in interpolating
...
environment_setup.add_radiation_pressure_interface(bodies, "Himalia", radiation_pressure_settings)
...
for file in raw_observation_files:
    ...
    with open(file, 'r') as f:
        ...
        for row in csv_reader:
            ...
            weights_plot.append(np.power(uncertainty_ra, -2))
            weight_plot_time.append(time/ (constants.JULIAN_DAY*365.25)+ 2000)
            #weight_number.append(n)
    # Final arc
    if arc_uncertainties_ra:
        ...
        avg_uncertainty_dec = sum(arc_uncertainties_dec) / len(arc_uncertainties_dec)
        n = len(arc_uncertainties_ra)
        noise_level.extend([avg_uncertainty_ra, avg_uncertainty_dec*np.sqrt(n)] * n)
    ...
    #         ObservationList.append(np.asarray([float(row[1]),float(row[2])]))
    #         obstimes.append(float(row[0]))
    #         uncertainty_right_ascension = float(row[3])
    #         uncertainty_declination = float(row[4])
    #         noise_level.append(uncertainty_right_ascension)
    #         noise_level.append(uncertainty_declination)
            # epoch_days = constants.JULIAN_DAY_ON_J2000 + float(row[0]) / constants.JULIAN_DAY
            # epoch = time_conversion.datetime_to_tudat(time_conversion.julian_day_to_calendar_date(epoch_days))
            # position = spice.get_body_cartesian_state_at_epoch('Himalia',"Earth",'ECLIPJ2000','LT',epoch.epoch())
            # position_correction = observatory_ephemerides.cartesian_state(epoch.epoch())
            # position_corrected = position-position_correction
            # spherical = element_conversion.cartesian_to_spherical(position)
            # RA_spice = spherical[2]
            # DEC_spice = spherical[1]
            # RA_comparisson.append(-RA_spice+float(row[1]))
    # plt.scatter(obstimes,RA_comparisson)
    # plt.grid()
    # plt.xlabel('time [s]')
    # plt.ylabel('RA obs-spice [rad]')
    # plt.show()
    ...
    #Plotting input
    # color = next(colors)
    # plt.scatter(2000+ np.asarray(times)/(365.25*3600*24),np.asarray(uncer)[:,0], color='#A50034', facecolors='none', marker=markers[0])
    # plt.scatter(2000+ np.asarray(times)/(365.25*3600*24),np.asarray(uncer)[:,1], color='#A50034', marker=markers[1])
#plt.scatter(weight_plot_time,weights_plot)
plt.scatter(weight_plot_time, weights_plot, c='#0d0887')
plt.grid()
plt.xlabel('Year')
plt.ylabel('Arc weight [rad$^{-2}$]')
plt.yscale('log')
plt.savefig('Weights.png', dpi=300, bbox_inches='tight')
plt.show()
#to get a sorted plot
# sorted_indices = np.argsort(study_labels)
# sorted_study_labels = np.array(study_labels)[sorted_indices]
# sorted_color_list = color_list[sorted_indices]

# legend2_handles = [
#     mlines.Line2D([], [], color=sorted_color_list[i], marker='o', linestyle='None', label=sorted_study_labels[i][:-4])
#     for i in range(len(study_labels))  # Use the list of colors here
# ]
# plt.plot([], [], color='#A50034', marker=markers[0], linestyle='None', label='RA', markersize=10, markeredgewidth=1.5)
# plt.plot([], [], color='#A50034', marker=markers[1], linestyle='None', label='DEC', markersize=10, markeredgewidth=1.5)
# plt.subplots_adjust(right=0.7)
# first_legend = plt.legend(handles=legend1_handles, loc='upper left', bbox_to_anchor=(1, 1),)
# ax = plt.gca().add_artist(first_legend)

# plt.legend(handles=legend2_handles, loc='center left', bbox_to_anchor=(1, 0.5), title='NSDC ID')
# plt.ylabel('Observed - Spice [rad]')
# plt.xlabel('Time')
# plt.legend()
# plt.grid()
# plt.savefig('Input_residuals.png', dpi=300, bbox_inches='tight')
# plt.plot()                         
...
acceleration_settings = {"Himalia": accelerations_settings_common}
...
for i in bodies_to_propagate:
    initial_state = [ 3.63053014e+09 , 9.37120006e+09, -2.05731927e+09 ,-2.70456109e+03, 1.99809264e+03 , 1.50790580e+03]
...
for time_step in time_steps:
    ...
    dependent_variables_to_save = [propagation_setup.dependent_variable.keplerian_state('Himalia', 'Jupiter'),
                                   propagation_setup.dependent_variable.relative_distance('Himalia', 'Earth')]
    ...
    inv_pre_v = 1/(precision/(3600*12))**2
    ...
    for epoch in state_history.keys():
        ...
        # position = spice.get_body_cartesian_state_at_epoch('Himalia',"Earth",'ECLIPJ2000','LT',epoch)
        # spherical = element_conversion.cartesian_to_spherical(position)
        # RA_spice = spherical[2]
        # DEC_spice = spherical[1]
        
        # position_J = spice.get_body_cartesian_state_at_epoch('Jupiter',"Earth",'ECLIPJ2000','LT',epoch)
        # position_H = np.vstack(list(state_history.values()))[k] + position_J
        # spherical_prop = element_conversion.cartesian_to_spherical(position_H)
        # RA_prop = spherical_prop[2]
        # DEC_prop = spherical_prop[1]

        # RAdif.append(RA_prop-RA_spice)
        # DECdif.append(DEC_prop-DEC_spice)

        k+=1
    state_history_difference = np.vstack(list(state_history.values())) - np.vstack(list(ephemeris_state_history.values()))
    position_difference = {'Himalia': state_history_difference[:, 0:3]}
    velocity_difference = {'Himalia': state_history_difference[:, 3:6]}
...
#          label=r'Amalthea2', c='#EC6842')
ax1.set_title(r'Difference in Position')
ax1.plot(time2plt, np.linalg.norm(position_difference['Himalia'], axis=1) * 1E-3,
         label=r'Himalia', c='#A50034')
ax1.xaxis.set_major_locator(mdates.YearLocator(20))
ax1.xaxis.set_minor_locator(mdates.YearLocator(2))
ax1.xaxis.set_major_formatter(mdates.DateFormatter('%Y'))
ax1.set_ylabel(r'Propagated - Spice [km]')
...
ax1.plot(time2plt, np.linalg.norm(velocity_difference['Himalia'], axis=1),
         label=r'Himalia', c='#A50034')
ax1.xaxis.set_major_locator(mdates.YearLocator(20))
ax1.xaxis.set_minor_locator(mdates.YearLocator(2))
ax1.xaxis.set_major_formatter(mdates.DateFormatter('%Y'))
...
#          label=r'Amalthea2', c='#EC6842')
ax1.set_title(r'Difference in Position')
ax1.plot(time2plt, np.linalg.norm(position_difference['Himalia'], axis=1)/propagation_kepler_elements[:,6],
...
axes[0,2].hist(residual_RA_3, bins=30,range=(-1*10**(-6), 1*10**(-6)), edgecolor='black')
...
axes[1,2].hist(residual_DEC_3, bins=30,range=(-1*10**(-6), 1*10**(-6)), edgecolor='black')
...
ax1.scatter(obstime2plt, residual_RA,
         label=r'Himalia RA', c='#A50034')
ax1.scatter(obstime2plt, residual_DEC,
         label=r'Himalia DEC', c='#0076C2')
ax1.xaxis.set_major_locator(mdates.YearLocator(20))
ax1.xaxis.set_minor_locator(mdates.YearLocator(2))
ax1.xaxis.set_major_formatter(mdates.DateFormatter('%Y'))
ax1.set_ylabel(r'Angular position residual [rad]')
...
# ax1.set_title(r'Angular position residual')
# ax1.scatter(obstime2plt, residual_RA,
#          label=r'RA observation residual', c='#A50034')
# ax1.plot(time2plt, RAdif,
#          label=r'RA propagated - Spice', c='#0076C2')
...
# ax1.grid()
# #ax1.set_yscale('log')
# plt.savefig(os.path.join(folder_name, "ResidualsANDposdiffRA.png"))
# plt.show()
...
# ax1.set_title(r'Angular position residual')
# ax1.scatter(obstime2plt, residual_DEC,
#          label=r'DEC observation residual', c='#A50034')
# ax1.plot(time2plt, DECdif,
#          label=r'DEC propagated - Spice', c='#0076C2')
# ax1.xaxis.set_major_locator(mdates.YearLocator(10))
# ax1.xaxis.set_minor_locator(mdates.YearLocator(1))
# ax1.xaxis.set_major_formatter(mdates.DateFormatter('%Y'))
# ax1.set_ylabel(r'Angular position residual [rad]')
...
# ax1.grid()
# #ax1.set_yscale('log')
# plt.savefig(os.path.join(folder_name, "ResidualsANDposdiffDEC.png"), dpi=300, bbox_inches='tight')
# plt.show()
